...
namespace Gpos700_XamrinAndroid.ExemploSAT.SatPages
{
    ...
    public class Teste : Activity
    {
        ...
        public void repostaSat(string operacao)
        {
            ...
            {
                case "ConsultarSat":
                    retornoOperacao = satFunctions.ConsultarSat(SatUtils.GerarNumeroSessao);
                    break;
                case "ConsultarStatusOperacional":
                    retornoOperacao = satFunctions.ConsultarStatusOperacional(SatUtils.GerarNumeroSessao, txtCodAtivacao.Text);
                    break;
                case "EnviarTesteFim":
                    retornoOperacao = satFunctions.EnviarTesteFim(txtCodAtivacao.Text, SatUtils.GerarNumeroSessao);
                    break;
                case "EnviarTesteVendas":
                    retornoOperacao = satFunctions.EnviarTesteVendas(txtCodAtivacao.Text, SatUtils.GerarNumeroSessao);
                    break;
                case "CancelarUltimaVenda":
                    retornoOperacao = satFunctions.CancelarUltimaVenda(txtCodAtivacao.Text, ultimaChaveVenda, SatUtils.GerarNumeroSessao);
                    break;
                case "ConsultarNumeroSessao":
                    retornoOperacao = satFunctions.ConsultarNumeroSessao(txtCodAtivacao.Text, Integer.ParseInt(codigoSessao), SatUtils.GerarNumeroSessao);
                    break;
                default:
                    retornoOperacao = "";
                    break;
